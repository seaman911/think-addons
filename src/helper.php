 ...
 * @return string
 */
function get_addon_class($name, $type = 'hook', $class = null, $addonmodule = null)
{
    ...
    switch ($type) {
        case 'controller':
            if($addonmodule){
                $namespace = "\\addons\\" . $name . "\\{$addonmodule}\\controller\\" . $class;
            }else{
        ...
        default:
            if($addonmodule){
                $namespace = "\\addons\\" . $name . "\\{$addonmodule}\\" . $class;
            }else{
...
function addon_url($url, $param = [], $suffix = true, $domain = false)
{
    ...
    $addonmodule = null;
    if($controller = trim($controller, '.') and strpos($controller, '.') !== false){
        list($addonmodule, $controller) = explode('.', $controller);
    }
    /* 解析URL带的参数 */
    if (isset($url['query'])) {
    ...
    // 生成插件链接新规则
    if($addonmodule){
        $actions = "{$addons}-{$addonmodule}-{$controller}-{$action}";
    }else{
